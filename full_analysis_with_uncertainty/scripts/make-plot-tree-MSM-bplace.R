...
# plot with thicker lines and points for cover art
    ggtree(tree, aes(color=new.branch.colours,size=new.branch.colours), layout='fan') + #%<+% d1 +
      geom_tippoint(aes(color=new.individual,shape=border), size=1.4) +
      geom_treescale(x=-0.01, y = length(tree$tip.label)*0.5,  offset=4) +
      scale_size_manual(values = c("Amsterdam non-MSM or\noutside Amsterdam" = 0.2,
                                   "Amsterdam MSM subgraphs" = 0.4),
                        na.value=0.2) +
      scale_shape_manual(values = c("Non-Amsterdam" = 20,
                                    "Born in the Netherlands" = 15,
                                    "Born outside the Netherlands" = 17),
      ...
      guides(col=guide_legend(ncol=1)) +
      ylim(-1, length(tree$tip.label) +1)
ggsave(file.path(outdir,glue("type_{subtype}_tree_newlabels_{trsm}_coloursubgraphs_tips_borders_bplace_fan_thicker_lines.pdf")), width = 8, height = tips*0.01, limitsize = F)
