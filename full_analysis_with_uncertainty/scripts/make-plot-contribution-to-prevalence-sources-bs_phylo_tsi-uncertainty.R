...
if(args$overwrite){saveRDS(po,file=paste0(outfile.base,'-flows_frombplace_adjusted_samplingbias_mcsamples_bs_phylo_tsi','.RDS'))}
...
g1 <- ggplot(subset(po,TO_BPLACE=='Overall')) + geom_bar(aes(x=FROM_BPLACE,y=M,fill=FROM_BPLACE),stat='identity',position=position_dodge(width=0.9)) +
  ...
  scale_y_continuous(labels = scales::label_percent(accuracy = 1L),breaks=seq(0,1,0.2))
if(args$overwrite){
  ggsave(file = paste0(outfile.base,'-contribution_to_flows_bs_phylo_tsi.pdf'), g1, w = 23, h = 10)
  ggsave(file = paste0(outfile.base,'-contribution_to_flows_bs_phylo_tsi.png'), g1, w = 16, h = 10)
}

### plot flows over prevelance contributions ----
cat(" \n --------------------------------  plot flows over prevalence contributions -------------------------------- \n")

po <- readRDS(file=paste0(outfile.base,'-flows_frombplace_adjusted_samplingbias_mcsamples_bs_phylo_tsi','.RDS'))
...
po <- dcast.data.table(po, FROM_BPLACE~stat, value.var = 'q')
po[, TO_BPLACE:= 'Overall']
if(args$overwrite){saveRDS(po,file=paste0(outfile.base,'-flows_frombplace_hivpos_bs_phylo_tsi','.RDS'))}
...
g2 <- ggplot(subset(po,TO_BPLACE=='Overall')) +
  ...
  theme(legend.pos='none',
        ...
        axis.text.x = element_text(angle=60, vjust = 0.95,hjust = 0.9)) #+ #,
if(args$overwrite){
  ggsave(file = paste0(outfile.base,'-contribution_to_flows_over_prev_bs_phylo_tsi.pdf'), g2, w = 23, h = 10)
  ggsave(file = paste0(outfile.base,'-contribution_to_flows_over_prev_bs_phylo_tsi.png'), g2, w = 16, h = 10)
}
...
g_r <- ggarrange(ggarrange(
  ...
  ggarrange(leg),ncol=1,nrow=2,align='hv',heights=c(0.7,0.3))
g <- ggarrange(g_l,g_r,ncol=2,align='hv',widths=c(0.35,0.65))

if(args$overwrite){
  ggsave(file = paste0(outfile.base,'-prev_flows_frombplace_contributions_panel_new_labs_bs_phylo_tsi.pdf'),
         g, w = 10, h = 5)
  ggsave(file = paste0(outfile.base,'-prev_flows_frombplace_contributions_panel_new_labs_bs_phylo_tsi.png'),
         g, w = 10, h = 5)
}
