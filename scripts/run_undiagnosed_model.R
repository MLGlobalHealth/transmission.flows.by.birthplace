...
saveRDS(dmap,file=file.path(outdir, paste0("mapping_georeg_id.RDS")))
dt <- merge(dt,dmap,by='mwmb')
N_diag <- subset(n_diag,TRANSM==args$trsm)

ps <- c(0.5,0.025,0.975)
p_labs <- c('M','CL','CU')
time_m_ci <- do[, list(q=quantile(time[INF_D>=2010 & INF_D<2016],prob=ps,na.rm=T),
...
time_m_ci_all <- do[, list(q=quantile(time[INF_D>=2010 & INF_D<2016],prob=ps,na.rm=T),
...
time_m_ci_total <- do[, list(q=quantile(time[INF_D>=2010 & INF_D<2016],prob=ps,na.rm=T),
														 q_label=p_labs)]
...
# model using cmdstan
model = rstan::stan_model(file.path(args$source_dir,'stan_models','undiagnosed_211102.stan'))
...
if(args$weights=='ECDC'){
	...
	mean_s <- ds[, list(av_undiagnosed=mean(p)),
							 by=c('trsm','mg','iter')]
	mean_s <- merge(mean_s,dmap,by.x='mg',by.y='mgid')
	setnames(mean_s,'mwmb','migrant_group')
	...
	ds[, L:= paste0(round(p0.5*100,2),'% [',round(p0.025*100,2),'-',round(p0.975*100,2),'%]')]
	ds <- merge(ds,dmap,by.x='mg',by.y='mgid')
	setnames(ds,'mwmb','migrant_group')
	#ds <- subset(ds,select=c('trsm','migrant_group','L'))
	saveRDS(ds,file=file.path(outdir,paste0('p_undiagnosed_average_',job_tag,"_",args$trsm,'.RDS')))
...
# calculate prob(undiagnosed) for a given month/year of infection
dat <- tidyr::crossing(year=seq(2014,2021,1),month=seq(1,12,1))
if(args$weights=='ECDC'){
	dat <- tidyr::crossing(year=seq(2014,2021,1))
}
ds <- merge(dat,ds,all=T)
ds <- data.table(ds)
if(args$weights=='ECDC'){
...
plot_msm <- ggplot(data=subset(du,trsm=='Amsterdam MSM' & !is.na(mlab)),aes(x=year,y=p0.5,fill=mlab)) +
	...
	scale_x_continuous(labels = seq(2014,2021,1),breaks=seq(2014,2021,1))  +
	coord_cartesian(ylim=c(0,1)) +
	labs(x='Year of infection',y="% infections undiagnosed by 2022",fill="Ethnicity") +
...
plot_hsx <- ggplot(data=subset(du,trsm=='Amsterdam heterosexuals' & !is.na(mlab)),aes(x=year,y=p0.5,fill=mlab)) +
	...
	scale_x_continuous(labels = seq(2014,2021,1),breaks=seq(2014,2021,1))  +
	coord_cartesian(ylim=c(0,1)) +
	labs(x='Year of infection',y="% infections undiagnosed by 2022",fill="") +
...
g_msm <- ggplot(subset(do,TRANSM %in% c('MSM') & INF_D>=2010 & INF_D<2015), aes(time)) +
...
g_hsx <- ggplot(subset(do,TRANSM %in% c('HSX') & INF_D>=2010 & INF_D<2015), aes(time)) +
	stat_ecdf(geom = "step",size=1.5,colour="black") +
