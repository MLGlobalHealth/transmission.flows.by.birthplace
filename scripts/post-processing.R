...
tmp <- paste0('Rscript ', file.path('$SCRIPT_DIR','scripts','check-convergence.R'),
              ' -source_dir $SCRIPT_DIR -stanModelFile $STAN_MODEL_FILE -out_dir $OUT_DIR -job_tag $JOB_TAG')
...
tmp <- paste0('Rscript ', file.path('$SCRIPT_DIR','scripts','make-plot-contribution-to-prevalence-flows.R'),
              ' -source_dir $SCRIPT_DIR -indir $IN_DIR -out_dir $OUT_DIR -analysis $ANALYSIS -undiagnosed $UNDIAG_JOB -overwrite $OVERWRITE')
cmd2 <- paste0(cmd2,tmp,'\n')
...
tmp <- paste0('Rscript ', file.path('$SCRIPT_DIR','scripts','make-table-characteristics.R'),
              ' -source_dir $SCRIPT_DIR -indir $IN_DIR -out_dir $OUT_DIR -analysis $ANALYSIS -undiagnosed $UNDIAG_JOB')
cmd2 <- paste0(cmd2,tmp,'\n')
