...
args <- list(
  ...
  outdir = '/rds/general/project/ratmann_roadmap_data_analysis/live/transmission_sources'
  )
...
infile.georeg <- file.path(args$indir,'misc','NEWGEO_220713.csv')
...
meta <- subset(tmp, select=c('ID','REP','ST','ST_CLADE','NAME','FULL_NAME','SELECT','CITY','BIRTH_YEAR','GENDER','TRANSM','HIV1_POS_D','RECART_D','SEQID','SAMPLING_DATE','ORIGIN','BIRTH_COUNTRY','mwmb'))
...
saveRDS(meta,file=file.path(args$outdir,'meta_data_mg_country.rds'))
...
saveRDS(age,file=file.path(args$outdir,'data_age.rds'))

# make infection date dataset
dt <- data.table(read.csv(file.path(args$indir,'Data','infection_time_estimates','220331_seq_update','roadmap_cd4_v3_est.csv')))

dt <- unique(subset(dt,select=c('id','hiv_pos_d','estsctodiagMedian','estsctodiagLL','estsctodiagUL')))
write.csv(dt,file.path(args$outdir,'Infection_date_est_rec.csv'))
